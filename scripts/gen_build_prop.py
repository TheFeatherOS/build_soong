...
def get_build_keys(product_config):
  ...
  if default_cert == "" or default_cert == os.path.join(TEST_KEY_DIR, "testKey"):
    return "test-keys"
  if default_cert.startswith("vendor/feather-priv/"):
    return "release-keys"
  return "dev-keys"
...
def parse_args():
  ...
  config["FeatherDevice"] = config["DeviceName"]
  config["FeatherDesc"] = config["BuildDesc"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] != "eng":
    ...
    print(f"ro.build.display.id?={config['FeatherDesc']}")
  ...
  print(f"ro.feather.device={config['FeatherDevice']}")
